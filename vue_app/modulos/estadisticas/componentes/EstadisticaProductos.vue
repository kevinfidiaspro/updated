<template>    
    <v-container>
        <v-toolbar color="red lighten-2" dark class="mx-auto">
            <v-toolbar-title>Estadisticas Productos</v-toolbar-title>
        </v-toolbar>
        <div class="background my-container">
            <v-row class="px-0">
                <v-col class="col-md-5">
                    ...
                    <v-col class="col-md-7 py-0 mt-4">
            ...
            <div>
                <v-col class="col-md-12 py-0 mt-4">
                    <v-data-table :headers="headers" :items="productData" item-key="id" 
                     mobile-breakpoint="0" :items-per-page="8" class="elevation-1 pa-0 ma-0 mt-4">
                    <!-- Total -->
                    <template v-slot:item.TotalVentaProducto="{ item }"> 
                        <span>{{item.TotalVentaProducto | fixed_n}}</span> €
                    </template>        
                    <template slot="body.append">
                        <tr >
                            ...
                            <th style="text-align: center;"><strong>{{ sumField('TotalVentaProducto') | fixed_n}} €</strong></th>
                        </tr>
                    </template>
                    </v-data-table>
                </v-col>
...
export default {
    ...
    data() {
        return {       
            ...
            [
                ...
                { text: 'Total € Productos', align: 'center', value: 'TotalVentaProducto', sortable: true }
            ],
            headers2: 
            [
    ...
    methods: {
        ...
        getProductData() {
            axios.get(`api/getproductdata/${this.rango.desde}/${this.rango.hasta}`)
            .then(res => {
        ...
        getCategoData() {
            axios.get(`api/getcategodata/${this.rango.desde}/${this.rango.hasta}`)
            .then(res => {
        ...
        sumField(key) {
            ...
            return this.productData.reduce((a, b) => a + (b[key] || 0), 0);
        },
    ...
    computed: {
        ...
        chartOptions() {
            return {
                title: { text: "ESTADISTICA PRODUCTOS", align: "center" },
