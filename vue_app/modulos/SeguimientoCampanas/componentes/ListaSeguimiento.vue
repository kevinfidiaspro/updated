<template>
    <v-card class="pa-3 ma-3">
        <v-toolbar flat color="#1d2735" dark>
            ...
            <pre><v-toolbar-title><h2>Listado de Seguimientos de Camapaña</h2></v-toolbar-title></pre>
        </v-toolbar>
        <loader v-if="isloading"></loader>
            ...
            <v-tooltip right>
            <template v-slot:activator="{ on, attrs }">
                <v-btn fab :to="{ path: `/guardar-seguimiento-campana` }" :loading="isloading" :disabled="isloading" color="orange darken-1" class="mt-2" v-bind="attrs" v-on="on">
        ...
        <v-data-table :headers="headers" :search="search" :items-per-page="10" :items="seguimientos" item-key="id" class="elevation-1">
            ...
            <template v-slot:item.action="{ item }">
                <router-link v-if="user.role == 1 || user.role == 2" :to="{ path: `/guardar-seguimiento-campana?id=${item.id}` }" class="action-buttons">
                   <v-icon
                        small
                        class="mr-2"
                        color="#1d2735"
                ...
                <router-link v-if="user.role == 1 || user.role == 2" :to="{ path: `/lista-fases-campana?id=${item.id}` }" class="action-buttons">
                   <v-icon
                        small
                        class="mr-2"
                        color="#1d2735"
...
<script>
    export default {
        data() {
            return {
                ...
                [
                    ...
                    {text: 'Fecha Desde', value: 'desde'},
                    {text: 'Fecha Hasta', value: 'hasta'},
        ...
        methods: {
            deletePlan() {
            axios
                .get(`api/delete-seguimiento/${this.selectedItem.id}`)
                .then(
                    (res) => {
            ...
            getAllSeguimientos() {
                axios.get('api/get-seguimientos').then(res => {
