<template>
    <v-card class="pa-3 ma-3">
        <v-toolbar flat color="#1d2735" dark>
            ...
            <pre><v-toolbar-title><h2>Listado de Paginas de Facebook</h2></v-toolbar-title></pre>
        </v-toolbar>
        ...
        <v-tooltip right>
            <template v-slot:activator="{ on, attrs }">
                <v-btn
                    fab
                    :to="{ path: `/guardar-pagina-facebook` }"
                    :loading="isloading"
                    :disabled="isloading"
        ...
        >
            ...
            <template v-slot:item.action="{ item }">
                <router-link
                    v-if="user.role == 1 || user.role == 7 || user.role == 2"
                    :to="{ path: `/guardar-pagina-facebook?id=${item.id}` }"
                ...
                <router-link
                    v-if="user.role == 1 || user.role == 7 || user.role == 2"
                    :to="{ path: `/lista-formulario-facebook?id=${item.id}` }"
                    class="action-buttons"
                >
                    <v-icon
                        small
...
export default {
    ...
    methods: {
        getAllformularios() {
            axios.get("webhook/get-pages").then(
                (res) => {
